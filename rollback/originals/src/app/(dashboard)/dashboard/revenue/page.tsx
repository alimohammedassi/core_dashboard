...
export default async function RevenuePage() {
  ...
  if (user) {
    ...
    // Fallback to local payment_intents if Stripe not configured
    if (isMock) {
      ...
      const allGross = (allPayments ?? []).reduce((s: number, r: { amount: number }) => s + (r.amount ?? 0), 0);
      const monthGross = (monthPayments ?? []).reduce((s: number, r: { amount: number }) => s + (r.amount ?? 0), 0);

      if (allGross > 0 || (allPayments && allPayments.length === 0 && !isStripe)) {
        // Use real DB values if table exists, else show mock numbers
        if (allGross > 0) {
          gross = allGross;
          isMock = false;
        } else {
          // mock fallback numbers for demo
          gross = 45800; // $458.00
          isMock = true;
        }
        // Still show month gross in card
      } else {
        gross = 45800;
      }
  ...
  if (isMock && !isStripe) {
    ...
    net = gross - commission;
    payouts = [
      { id: "po_mock_1", amount: 12000, currency: "usd", arrival_date: Math.floor(Date.now() / 1000) - 86400 * 2, status: "paid" },
      { id: "po_mock_2", amount: 18500, currency: "usd", arrival_date: Math.floor(Date.now() / 1000) - 86400 * 10, status: "paid" },
    ];
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {isMock ? <Badge variant="outline" className="bg-amber-50 border-amber-200 text-amber-700">Mock estimate — 15% fee</Badge> : <Badge>Live Stripe data</Badge>}
      </div>
