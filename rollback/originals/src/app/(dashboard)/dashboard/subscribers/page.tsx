...
function StatusBadge({ status }: { status: string }) {
  const variant =
    status === "active" ? "default" : status === "canceled" || status === "expired" ? "destructive" : "secondary";
...
export default async function SubscribersPage({
  ...
  if (user) {
    ...
    if (error || !data || data.length === 0) {
      ...
      isMock = true;
      rows = (filter ? mockSubscriptions.filter((s) => s.status === filter) : mockSubscriptions) as unknown as typeof rows;
    } else {
      rows = data as unknown as typeof rows;
    }
  } else {
    rows = mockSubscriptions as unknown as typeof rows;
    isMock = true;
  }

  const statuses = ["active", "canceled", "past_due", "trialing", "expired", "paused"] as const;

  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h1 className="text-2xl font-semibold tracking-tight">Subscribers</h1>
        {isMock && <Badge variant="outline">Mock data</Badge>}
      </div>
